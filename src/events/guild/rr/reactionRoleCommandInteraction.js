        ...
        case "group":
          {
            ...
            bot.database.query(query, function (err, result) {
              ...
              const filtered = groupNames.filter((groupName) =>
                groupName.toLowerCase().startsWith(focusedValue)
              );
              interaction.respond(
        ...
        case "role_to_remove":
          {
            ...
            const filtered = roleNames
              .filter((roleName) => roleName !== null)
              .filter((roleName) =>
                roleName.toLowerCase().startsWith(focusedValue)
              );
            interaction.respond(
              filtered.map((choice) => ({ name: choice, value: choice }))
            );
        ...
        case "rrGroupDelete":
          {
            ...
            bot.database.query(
              `DELETE FROM reactiongroups WHERE serverId=${interaction.guildId} AND groupName='${groupName}'`,
              function (err, result) {
                if (err) {
        ...
        case "rrGroupView":
          {
            const groupName = interaction.values[0];
            const embed = utils
              .getDefaultMessageEmbed(bot, { title: "Reaction Roles" })
            ...
            interaction
              .update({ embeds: [embed], fetchReply: true })
              .then((message) => {
                ...
                bot.database.query(query, function (err, result) {
                  ...
                  if (hasRoles) {
                    ...
                    embed.addFields({
                      name: `Reaction Roles`,
                      value: `No reaction roles have been set up for this group`,
                      inline: true,
                    });
                  }
      ...
      switch (interaction.customId) {
        case "rrCorrect":
          {
            ...
            bot.database.query(
              `INSERT INTO reactiongroups (serverId, groupName) VALUES (${interaction.guildId}, '${groupName}')`,
              function (err, result) {
                if (err) {
                  interaction.reply({
                    embeds: [
        ...
        case "rrIncorrect":
          const embed = utils
            ...
            .addFields({
              name: "Cancelled Adding Group",
              value: "To try adding a group again, use the command again!",
            });
        ...
        case "rrAcceptAddRole":
          {
            ...
            if (roleId == null) {
              ...
              embed.addFields({
                name: "Invalid Role",
                value: `${roleMention} isn't a valid role in this server!`,
              });
            }
            if (!isUnicode) {
              ...
              if (interaction.guild.emojis.cache.get(parsedReacc.id) == null) {
                ...
                embed.addFields({
                  ...
                  value: `The emoji ${reacc} isn't a valid emoji in this server!`,
                });
              }
            }
            ...
            bot.database.query(
              `SELECT rGroupKey FROM reactiongroups WHERE serverId='${interaction.guildId}' AND groupName='${groupName}'`,
              function (err, res) {
                const rGroupKey = res[0]["rGroupKey"];

                bot.database.query(
                  `SELECT reaction, roleId FROM reactionroles WHERE rGroupKey=${rGroupKey}`,
                  function (err, results) {
                        ...
                        embeds: [
                          utils
                            ...
                            .addFields({
                              ...
                              value: `The reaction ${reacc} is already a reaction for the group ${bold(
                                groupName
                              )}`,
                            }),
                        ],
                        ...
                        embeds: [
                          utils
                            ...
                            .addFields({
                              ...
                              value: `The role <@&${roleId}> is already a role for the group ${bold(
                                groupName
                              )}`,
                            }),
                        ],
                    ...
                    bot.database.query(
                      `INSERT INTO reactionroles (rGroupKey, reaction, roleId) VALUES ('${rGroupKey}', '${reacc}', '${roleId}')`,
                      function (err, result) {
                        if (err) {
                          return utils.raiseError(bot, err);
                        }
                        interaction.update({
                          embeds: [
                            utils
                              ...
                              .addFields({
                                name: "Reaction Role Added",
                                value: `The role ${roleMention} has been added to ${bold(
                                  groupName
                                )} with the reaction ${reacc}`,
                              }),
                          ],
                          components: [],
                        });
        ...
        case "rrAcceptRemoveRole":
          {
            ...
            bot.database.query(
              `SELECT rGroupKEY FROM reactiongroups WHERE groupName='${groupName}' AND serverId='${interaction.guildId}'`,
              function (err, results) {
                const rGroupKEY = results[0]["rGroupKey"];

                bot.database.query(
                  `SELECT * FROM reactionroles WHERE rGroupKEY='${rGroupKEY} AND roleId='${roleId}'`,
                  function (err, results) {
