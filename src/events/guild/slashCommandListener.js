...
module.exports = {
  ...
  callback: async (bot, interaction) => {
    ...
    // if (!perms.checkIsOwner(interaction.member)) {
    //   return interaction.reply({
    //     content: "Commands are currently disabled for testing purposes",
    //     ephemeral: true,
    //   });
    // }
    ...
    if (cmd.length != 0) {
      ...
      if (command) {
        ...
        const results_roleperms = await bot.asyncQuery(
          `SELECT roleId FROM rolePermissions WHERE serverId=? AND commandName=?`,
          [interaction.guildId, command.name]
        );
        const roleIds = results_roleperms.map((row) => row.roleId);
        commandPermission |=
          !results_roleperms.length |
          interaction.member.roles.cache.some((r) => roleIds.includes(r.id));

        // Checking for user perms
        const results_userperms = await bot.asyncQuery(
          `SELECT userId FROM memberPermissions WHERE serverId=? AND commandName=?`,
          [interaction.guildId, command.name]
        );
        const userIds = results_userperms.map((row) => row.userId);
        commandPermission |=
          !results_userperms.length | userIds.includes(interaction.member.id);

        // Checking for hard-coded permissions
        commandPermission &=
          command.permissions.length > 0
            ? command.permissions.some((check) => check(interaction.member))
            : commandPermission;
        // postQuery = new Date();
