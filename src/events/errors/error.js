...
module.exports = {
  ...
  callback: async (bot, error) => {
    ...
    if (error instanceof PermissionError) {
      ...
      permErrResponseEmbed.setImage(
        "https://c.tenor.com/c5a_h8U1MzkAAAAC/tenor.gif"
      );
    ...
    if (error.interaction) {
      const standardErrResponseEmbed = utils.getDefaultMessageEmbed(bot, {
        title: "Error",
        description: utils.emptyEmbed,
        color: colors.FireBrick,
      });
      standardErrResponseEmbed.addFields({
        name: error.name,
        value: error.message,
      });
