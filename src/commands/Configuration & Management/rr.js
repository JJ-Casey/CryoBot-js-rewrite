  ...
  return new Promise((resolve, reject) => {
    ...
    bot.database.query(query, [group], function (err, results) {
      ...
      const filteredResults = results.filter(
        (row) => (row["reaction"] != null) & (row["roleId"] != null)
      );
...
module.exports = {
  ...
  slash: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName("remove").setDescription("Delete reaction role group")
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName("addrole")
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName("sendmessage")
        .setDescription(
          "Send the reaction group message to the specified channel"
        )
        .addStringOption((option) =>
          option
            .setName("group")
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName("setmessage")
        .setDescription(
          "Set the message to be used as the reaction group message"
        )
        .addStringOption((option) =>
          option
            .setName("group")
  ...
  run: async (bot, interaction) => {
    ...
    switch (interaction.options.getSubcommand()) {
      case "view":
        {
          ...
          embed.addFields({
            ...
            value: "Select a group to view information about it!",
          });
          interaction
      ...
      case "create":
        ...
        const groupNameRow = new ActionRowBuilder().addComponents(
          groupNameInput
        );
        const groupDescRow = new ActionRowBuilder().addComponents(
          groupDescInput
        );
      ...
      case "remrole":
        {
          ...
          const embed = utils
            .getDefaultMessageEmbed(bot, { title: "Remove Role" })
            .setColor(colors.Orange);

          const results = await bot.asyncQuery(
            `SELECT reaction FROM reactionGR WHERE groupName='${groupName}' AND serverId='${interaction.guildId}' AND roleId='${role.id}'`
          );
      ...
      case "sendmessage":
        ...
        try {
          ...
          const temp = await createReactionEmbed(
            bot,
            groupName,
            interaction.guildId
          );
          ...
          const resp = utils.getDefaultMessageEmbed(bot, {
            title: "Reaction Message Sent",
          });
          ...
          resp.setDescription(
            `The message was sent to ${channel}.\n${urlToMessage}`
          );
      ...
      case "setmessage":
        ...
        embed.setDescription(
          `Setting the reaction message for group **${groupName}**`
        );
        ...
        if (messageURL == null) {
          embed.addFields({
            name: "Please enter the link to the message in chat",
            value:
              "Copy the link to the message that you want to set as the reaction message",
          });
          embed.setImage("https://i.imgur.com/AxQ9Sbg.png");

          messageURL = await new Promise((resolve) => {
            interaction.editReply({ embeds: [embed] }).then(() => {
              const message_filter = (m) => m.author.id == interaction.user.id;
              interaction.channel
                .awaitMessages({
                  ...
                  collected
                    ...
                    .then(() => {
                      if (response.toLowerCase() == "yes") {
                        ...
                        bot.database.query(
                          `INSERT INTO reactionmessages (serverId, channelId, messageId) VALUES (?,?,?)`,
                          ...
                          function (err, result) {
                              ...
                              return interaction.editReply({
                                embeds: [
                                  utils.getDefaultMessageEmbed(bot, {
                                    title: "Set Message",
                                    description:
                                      "The message has not been set!\nThat message already belongs to a group!",
                                    color: colors.FireBrick,
                                  }),
                                ],
                              });
                            ...
                            bot.database.query(
                              `SELECT MAX(rMessageKEY) FROM reactionmessages AS pKey`,
                              function (err, results) {
                                if (err) {
                                  console.err(err);
                                }
                                const rMessageKEY =
                                  results[0]["MAX(rMessageKEY)"];

                                bot.database.query(
                                  `UPDATE reactiongroups SET rMessageKEY=? WHERE serverId=? AND groupName=?`,
                                  [rMessageKEY, interaction.guildId, groupName]
                                );
                              }
                            );
                          }
                        ...
                        interaction.editReply({
                          embeds: [
                            utils.getDefaultMessageEmbed(bot, {
                              title: "Set Message",
                              description: "The message has not been set!",
                            }),
                          ],
                        });
                      }
            ...
            interaction
              .editReply({
                embeds: [
                  utils.getDefaultMessageEmbed(bot, {
                    title: "Set Message",
                    description: "That doesn't look like a valid message url!",
                  }),
                ],
              })
              .then((m) => {
