...
module.exports = {
  ...
  run: async (bot, interaction) => {
    ...
    for (const slashCommand of bot.slashCommands.values()) {
      if (
        slashCommand.name != "deployCommands" ||
        slashCommand.name != "unDeployCommands"
      )
        continue;
    ...
    (async () => {
      try {
        const embed = utils
          ...
          .addFields({
            name: "Refreshing",
            value: `Removing all application (/) commands.`,
          });
          ...
          rest
            ...
            .then((data) => {
              embed
                .setFields({
                  name: "Refreshed",
                  value: `Successfully removed application (/) commands.`,
                })
