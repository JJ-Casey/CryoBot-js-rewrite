const { ChatInputCommandInteraction, SlashCommandBuilder, Collection, PermissionFlagsBits } = require('discord.js');
...
module.exports = {
    ...
    slash: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('reason')
                .setDescription('The reason for their decimation'))
    ...
    run: async (bot, interaction) => {
        ...
        if (target_role_intersection.length > 0) {
            const responseEmbed = utils.getDefaultMessageEmbed(bot, { title: 'Nuh uh', color: colors.FireBrick });
            return interaction.reply({ embeds: [responseEmbed], ephemeral: true });
        }
        ...
        const responseEmbed = utils.getDefaultMessageEmbed(bot, { title: '', description: 'Luxx has decimated another mortal!', color: colors.FireBrick });

        const gifs = ['https://media.tenor.com/kQryFMu7fhoAAAAC/rezero-rem.gif', 'https://media.tenor.com/XQiSjsnR2g8AAAAC/rem-re-zero.gif']
        ...
        interaction.guild.members.fetch('235467134581342208')
            .then(luxx => {
                ...
                const logEmbed = utils.getDefaultMessageEmbed(bot, { title: '', description: 'Luxx has decimated another mortal!' });
                logEmbed.addFields(
                ...
                bot.database.query("SELECT channelId FROM log_channels WHERE logName='BOT_DEBUG' AND serverId=?", [interaction.guildId], function (err, result) {
                    if (err) { throw err; }
